            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">学习课件收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="kejianName" id="kejianName" placeholder="视频标题" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="kejianTypes" id="kejianTypes" lay-filter="kejianTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in kejianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../kejian/detail.jsp?id='+item.kejianId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.kejianPhoto?item.kejianPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../kejian/detail.jsp?id='+item.kejianId)"><span>{{item.kejianName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.kejianNewMoney"><span style="color: red;">{{item.kejianNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteKejianCollection(item.id)">删除 {{item.kejianName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 kejianCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            kejianCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteKejianCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('kejianCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#kejianName').val() != null && jquery('#kejianName').val() != "" && jquery('#kejianName').val() != "null" ) {
                param['kejianName'] = jquery('#kejianName').val();
            }
                     
            var kejianTypes = $("#kejianTypes").find("option:selected").val();
            if (kejianTypes != null && kejianTypes != "" && kejianTypes != "null") {
                            param['kejianTypes'] = kejianTypes;
                        }
                                                                                                    
            // 当前表
                                                    // 获取列表数据
            http.request('kejianCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function kejianTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kejian_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.kejianTypesList = res.data.list;
               }
           });
       }
