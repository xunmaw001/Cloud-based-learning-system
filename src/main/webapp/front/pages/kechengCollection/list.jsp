            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">课程信息收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="kechengName" id="kechengName" placeholder="课程名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="kechengTypes" id="kechengTypes" lay-filter="kechengTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in kechengTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../kecheng/detail.jsp?id='+item.kechengId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.kechengPhoto?item.kechengPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../kecheng/detail.jsp?id='+item.kechengId)"><span>{{item.kechengName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.kechengNewMoney"><span style="color: red;">{{item.kechengNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteKechengCollection(item.id)">删除 {{item.kechengName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 kechengCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            kechengCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteKechengCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('kechengCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#kechengName').val() != null && jquery('#kechengName').val() != "" && jquery('#kechengName').val() != "null" ) {
                param['kechengName'] = jquery('#kechengName').val();
            }
                     
            var kechengTypes = $("#kechengTypes").find("option:selected").val();
            if (kechengTypes != null && kechengTypes != "" && kechengTypes != "null") {
                            param['kechengTypes'] = kechengTypes;
                        }
                                                                                                                        
            // 当前表
                                                    // 获取列表数据
            http.request('kechengCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function kechengTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kecheng_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.kechengTypesList = res.data.list;
               }
           });
       }
