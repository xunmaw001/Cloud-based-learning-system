            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">学习视频收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shipinName" id="shipinName" placeholder="视频标题" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="shipinTypes" id="shipinTypes" lay-filter="shipinTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in shipinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../shipin/detail.jsp?id='+item.shipinId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.shipinPhoto?item.shipinPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../shipin/detail.jsp?id='+item.shipinId)"><span>{{item.shipinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.shipinNewMoney"><span style="color: red;">{{item.shipinNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteShipinCollection(item.id)">删除 {{item.shipinName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 shipinCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            shipinCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteShipinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shipinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#shipinName').val() != null && jquery('#shipinName').val() != "" && jquery('#shipinName').val() != "null" ) {
                param['shipinName'] = jquery('#shipinName').val();
            }
                     
            var shipinTypes = $("#shipinTypes").find("option:selected").val();
            if (shipinTypes != null && shipinTypes != "" && shipinTypes != "null") {
                            param['shipinTypes'] = shipinTypes;
                        }
                                                                                                    
            // 当前表
                                                    // 获取列表数据
            http.request('shipinCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function shipinTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shipin_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.shipinTypesList = res.data.list;
               }
           });
       }
