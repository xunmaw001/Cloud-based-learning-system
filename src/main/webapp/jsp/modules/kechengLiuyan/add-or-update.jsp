    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑课程信息留言</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">课程信息留言管理</li>
                        <li class="breadcrumb-item active">编辑课程信息留言</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">课程信息留言信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>负责教师</label>
                                        <input id="kechengJiaoshi" name="kechengJiaoshi" class="form-control"
                                               placeholder="负责教师" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>课程时间</label>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>留言内容</label>
                                        <script id="kechengLiuyanContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var kechengLiuyanContentUe = UE.getEditor('kechengLiuyanContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="kechengLiuyanContent" id="kechengLiuyanContent-input">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>回复内容</label>
...
<script>
    ...
    function upload() {
        ...
        $('#kechengLiuyanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('kechengLiuyanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("kechengLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addkechengLiuyan');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.kechengLiuyanContent != null && ruleForm.kechengLiuyanContent != 'null' && ruleForm.kechengLiuyanContent != '') {

            var kechengLiuyanContentUeditor = UE.getEditor('kechengLiuyanContentEditor');
            kechengLiuyanContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.kechengLiuyanContent != null){
                    mes = ''+ ruleForm.kechengLiuyanContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                kechengLiuyanContentUeditor.setContent(mes);
            });
        }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
            var kechengLiuyanContentEditor = UE.getEditor('kechengLiuyanContentEditor');
            if (kechengLiuyanContentEditor.hasContents()) {
                $('#kechengLiuyanContent-input').attr('value', kechengLiuyanContentEditor.getPlainTxt());
            }
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                kechengId: "required",
                yonghuId: "required",
                kechengLiuyanContent: "required",
                replyContent: "required",
                insertTime: "required",
            },
            messages: {
                kechengId: "课程信息id不能为空",
                yonghuId: "用户id不能为空",
                kechengLiuyanContent: "留言内容不能为空",
    ...
    // 获取当前详情
    function getDetails() {
        var addkechengLiuyan = window.sessionStorage.getItem("addkechengLiuyan");
        if (addkechengLiuyan != null && addkechengLiuyan != "" && addkechengLiuyan != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("kechengLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#kechengLiuyanContent").val(ruleForm.kechengLiuyanContent);
    ...
    function kechengDataBind(){
        ...
        $("#kechengJiaoshi").val(ruleForm.kechengJiaoshi);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            var kechengLiuyanContentUeditor = UE.getEditor('kechengLiuyanContentEditor');
            kechengLiuyanContentUeditor.ready(function () {
                kechengLiuyanContentUeditor.setDisabled('fullscreen');
            });
        ...
        if(window.sessionStorage.getItem('addkechengLiuyan') != null && window.sessionStorage.getItem('addkechengLiuyan') !=""){//新增查询,修改不查
            // 查询当前登录账户的信息
            var userId = window.sessionStorage.getItem('userId');
